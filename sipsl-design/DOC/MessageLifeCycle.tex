...
Invite from A does not move anymore and is locked. 

\
\framebox{Rule: Locked message can't be sent out by state machines, action
executer shall assert it the message is locked} 

\framebox{Rule: If a message is directed to a SM and storage is not needed, the
SM shall set it to SODE\_KILL}

\subsubsection{Step 4 action executer} 
\begin{itemize}
   \item Invite\_ALO executed in VALO
	\begin{itemize}
	  \item VALO duplicates INVITE
	  \item New Invite\_valo source message esplicitely set to Invite\_alo source
	  message
	  \item VALO set gen = VALO, dest = SM\_CL
	  \item VALO set genmod = modsource 
	  \item VALO set gencid = cidsource
	  \item VALO send INVITE\_valo to SL\_CC
	\end{itemize}
   \item action executer deletes Invite\_alo
   \item 100 Trying is sent to network
   \item timer\_s is sent to alarm 
\end{itemize}

\framebox{Rule: VALO must duplicate message}
\framebox{Rule: action executer must delete message after returning from valo}
\framebox{Rule: when a message is duplicated, the genmod and gencid are
automatically set}
\framebox{Rule: when a message is duplicated, the sourcemess is not set, it must
be escplicitely set, default value is MainMessage. This is needed because if we
delete the source message we are not able to change all messages pointing to
this.}

\framebox{Rule: SL\_CC will delete the message, if not locked due to a
creation of a SM}

\subsubsection{Step 5: transport} 
\begin{itemize}
   \item 100 Trying is sent to network
   \item 100 Trying is purged 
\end{itemize}
